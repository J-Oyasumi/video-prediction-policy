...
class VPP_Policy(nn.Module):#pl.LightningModule):
    ...
    def training_step(self, dataset_batch: Dict[str, Dict]) -> torch.Tensor:
        """Compute and return training loss (MSE over full action)."""
    ...
    @torch.no_grad()
    def validation_step(self, dataset_batch: Dict[str, Dict], print_it: bool = False) -> Dict[str, torch.Tensor]:
        """Compute validation loss (MSE over full action). Optional print_it for debug."""
        output = {}
        predictive_feature, latent_goal = self.extract_predictive_feature(dataset_batch)
        ...
        output["validation_loss"] = torch.nn.functional.mse_loss(action_pred, dataset_batch["actions"])
        if print_it and "frame_ids" in dataset_batch and "ann_file" in dataset_batch:
            import numpy as np
            frame_ids = np.array([idx.cpu().numpy() for idx in dataset_batch["frame_ids"]])
            ad = self.action_dim
