...
class Dataset_mix(Dataset):
    ...
    def _get_frames(self, label, frame_ids, cam_id, pre_encode, video_dir, load_cond_img=True):
    ...
    def _get_obs(self, label, frame_ids, cam_id, pre_encode, video_dir, load_cond_img=False):
        assert cam_id is not None
        temp_cam_id = cam_id
        frames = self._get_frames(label, frame_ids, cam_id = temp_cam_id, pre_encode = pre_encode, video_dir=video_dir, load_cond_img=load_cond_img)
    ...
    def __getitem__(self, index, cam_id = None, return_video = False):
        ...
        if 'calvin' in ann_file or 'xhand' in ann_file:
            ...
            cam_id = np.random.choice([0,1], p=[0.7,0.3])
        ...
        if self.args.pre_encode:
            ...
            if return_video:
                video, cam_id = self._get_obs(label, frame_ids, cam_id, pre_encode=False,video_dir=sampled_video_dir)
                data['video'] = video.float()
        else:
            video, cam_id = self._get_obs(label, frame_ids, cam_id, pre_encode=False, video_dir=sampled_video_dir)
            data['video'] = video.float()
            data['latent'] = video.float()
